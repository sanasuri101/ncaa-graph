...
function onNetworkClick(params) {
  ...
  // On mobile, open the bottom sheet and switch to Detail tab
  if (window.innerWidth <= 768 && (params.nodes.length > 0 || params.edges.length > 0)) {
    const aiPanel = document.getElementById('ai-panel');
    if (aiPanel) {
      aiPanel.classList.add('mob-open');
      // Activate detail tab
      ...
      const detailBtn = document.querySelector('.ai-tab[data-mode="detail"]');
      if (detailBtn) detailBtn.classList.add('active');
      let mobPanel = document.getElementById('mob-panel-detail');
      if (mobPanel) {
        const sidebarContent = document.getElementById('tab-detail');
...
function focusTeam(id) {
  ...
  // On mobile, open the bottom sheet on the detail tab
...
function initMobileUI() {
  ...
  const aiPanel = document.getElementById('ai-panel');
  if (!aiPanel || aiPanel._mobInit) return;
  aiPanel._mobInit = true;

  const SHEET_H = () => window.innerHeight * 0.82;

  // ── Drag to open/close ──────────────────────────────────────────────────
  const dragHandle = document.getElementById('mob-drag-handle');
  const scoutHdr   = aiPanel.querySelector('.ai-scout-header');

  let dragStartY = 0, dragStartOpen = false, isDragging = false, didDrag = false;

  function startDrag(y) {
    dragStartY    = y;
    dragStartOpen = aiPanel.classList.contains('mob-open');
    isDragging    = true;
    didDrag       = false;
  ...
  function moveDrag(y) {
    ...
    const closed = SHEET_H() - 52;
    const base   = dragStartOpen ? 0 : closed;
    const next   = Math.max(0, Math.min(closed, base + dy));
    aiPanel.style.transform = `translateY(${next}px)`;
  }
  function endDrag(y) {
    if (!isDragging) return;
    isDragging = false;
    ...
    if (!didDrag) {
      // tap — toggle
      aiPanel.classList.toggle('mob-open');
    } else if (dragStartOpen && dy > 60) {
      aiPanel.classList.remove('mob-open');
    } else if (!dragStartOpen && dy < -60) {
      aiPanel.classList.add('mob-open');
    } else {
      // snap back
      if (dragStartOpen) aiPanel.classList.add('mob-open');
      else aiPanel.classList.remove('mob-open');
    }
  }

  [dragHandle, scoutHdr].forEach(el => {
    if (!el) return;
    ...
    el.addEventListener('touchend',   e => { endDrag(e.changedTouches[0].clientY); e.preventDefault(); }, { passive: false });
  });

  // ── Mobile tab switching — syncs sidebar content into panel ────────────
  // Tabs: chat, stats (native ai-panel tabs)
  // Tabs: overview, bracket, rankings, detail (mirror sidebar content)
  const MOB_SIDEBAR_TABS = ['overview', 'bracket', 'rankings', 'detail'];
  ...
  document.querySelectorAll('.ai-tab').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (MOB_SIDEBAR_TABS.includes(mode)) {
        // Show the corresponding sidebar tab content inside a mobile panel
        let mobPanel = document.getElementById(`mob-panel-${mode}`);
        if (mobPanel) {
          // Clone fresh content from the sidebar tab each time
          const sidebarContent = document.getElementById(`tab-${mode}`);
          if (sidebarContent) {
            mobPanel.innerHTML = sidebarContent.innerHTML;
            // Re-init bracket if needed
            if (mode === 'bracket' && typeof initBracket === 'function') initBracket();
          }
          mobPanel.classList.add('active');
        }
      } else {
        // Native panel (chat or stats)
  ...
  // ── Open sheet when clicking graph node/edge (handled in onNetworkClick) ─

  // ── AI Scout button in header opens sheet ──────────────────────────────
  const aiToggle = document.getElementById('ai-toggle');
  if (aiToggle) {
    aiToggle.addEventListener('click', () => {
      if (window.innerWidth <= 768) aiPanel.classList.add('mob-open');
    });
  }
}
...
function initMobileFirstImpression() {
  ...
  // Default to Overview tab on mobile — it's the most useful first view
  document.querySelectorAll('.ai-tab').forEach(b => b.classList.remove('active'));
  document.querySelectorAll('.ai-mode-content').forEach(p => p.classList.remove('active'));
  ...
  if (mobOverview) {
    ...
    if (sidebarOverview) mobOverview.innerHTML = sidebarOverview.innerHTML;
