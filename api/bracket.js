...
function getData() {
  ...
  const graph = JSON.parse(
    readFileSync(
      join(process.cwd(), "public", "data", "graph_data.json"),
      "utf8",
    ),
  );
  const torvik = JSON.parse(
    readFileSync(
      join(process.cwd(), "public", "data", "torvik_stats.json"),
      "utf8",
    ),
  );
  const form = JSON.parse(
    readFileSync(
      join(process.cwd(), "public", "data", "recent_form.json"),
      "utf8",
    ),
  );
  ...
  try {
    injuries = JSON.parse(
      readFileSync(
        join(process.cwd(), "data", "injury_overrides.json"),
        "utf8",
      ),
    );
  } catch {}
  ...
  for (const [espnId, override] of Object.entries(injuries.overrides ?? {})) {
    ...
    const projAdj = tv.proj_barthag
      ? Math.max(
          ...
          Math.min(
            0.99,
            sigmoid(k * (Math.max(0.01, logit(tv.proj_barthag) / k) - penalty)),
          ),
        )
      : adjBarthag;
    injuryMap[espnId] = {
  ...
  try {
    oppBarthag =
      JSON.parse(
        readFileSync(join(process.cwd(), "data", "opp_barthag.json"), "utf8"),
      ).opp_barthag ?? {};
  } catch {}

  const actualResults = buildActualResults(graph);
...
function buildActualResults(graph) {
  let allGames = [];
  try {
    allGames = JSON.parse(
  ...
  try {
    const cfg = JSON.parse(
      readFileSync(join(process.cwd(), "data", "bracket_config.json"), "utf8"),
    );
...
const LOGIT_MIN = -1.1; // logit(~0.25) low-major floor
const LOGIT_MAX = 4.04; // logit(~0.981) elite ceiling
function qualMult(b) {
  const bv = Math.max(0.001, Math.min(0.999, b ?? 0.7));
...
function decayWinRate(teamId, form, torvik, oppBarthag) {
  ...
  for (const g of games) {
    const recencyW = Math.exp(
      (-DECAY_LAMBDA * (DECAY_TODAY - new Date(g.date))) / 86400000,
    );
    const b =
      torvik?.teams?.[g.opp_id]?.torvik?.barthag ??
      oppBarthag?.[g.opp_id] ??
      0.7;
...
function wabAdj(tvA, tvB) {
  return Math.max(
    -0.04,
    Math.min(0.04, ((tvA.wab ?? 0) - (tvB.wab ?? 0)) / 270),
  );
}
...
function fourFactorsAdj(tvA, tvB) {
  ...
  const efgA = tvA.efg ?? 50,
    efgB = tvB.efg ?? 50;
  adj += ((efgA - efgB) / 100) * 0.15;

  // 2. TOV% edge — lower TOV rate is better (~25% weight)
  const tovA = tvA.tov_rate ?? 18,
    tovB = tvB.tov_rate ?? 18;
  adj += ((tovB - tovA) / 100) * 0.1; // positive when A turns it over less

  // 3. ORB% edge (~20% weight)
  const orbA = tvA.orb_rate ?? 30,
    orbB = tvB.orb_rate ?? 30;
  ...
  const tpaA = tvA.three_pa_rate ?? 35,
    tpaB = tvB.three_pa_rate ?? 35;
  const threePctA = tvA.three_p ?? 33,
    threePctB = tvB.three_p ?? 33;
...
function evidenceWinProb(
  ...
  const tvA = injuryMap?.[teamA.id]
    ? {
        ...rawTvA,
        adj_em: injuryMap[teamA.id].adj_em,
        barthag: injuryMap[teamA.id].barthag,
        proj_barthag: injuryMap[teamA.id].proj_barthag,
      }
    : rawTvA;
  const tvB = injuryMap?.[teamB.id]
    ? {
        ...rawTvB,
        adj_em: injuryMap[teamB.id].adj_em,
        barthag: injuryMap[teamB.id].barthag,
        proj_barthag: injuryMap[teamB.id].proj_barthag,
      }
    : rawTvB;
  ...
  const base = 0.2 * bP + 0.15 * pB + 0.25 * lP + 0.2 * sP + 0.2 * dP;
  const ffAdj = fourFactorsAdj(tvA, tvB);
  const evidAdj = Math.max(
    -0.12,
    Math.min(0.12, transAdj + wAdj + lAdj + ffAdj),
  );
  const compress = round <= 1 ? 0.1 : round <= 2 ? 0.06 : 0.02;
...
function winProb(
  ...
  if (model === "barthag") {
    ...
    const histWeight = Math.min(0.9, 0.5 + seedGap * 0.04); // scales 50%→90% with seed gap
    ...
    const lAdj = luckAdj(tvA, tvB);
    const wAdj = wabAdj(tvA, tvB);
    const allAdj = Math.max(
      -0.06,
      Math.min(0.06, formAdj + ffAdj + lAdj + wAdj),
    );
    ...
    if (adjusted >= 0.85) {
      ...
      const compression = round <= 1 ? 0.2 : round <= 2 ? 0.14 : 0.08;
    ...
    const seedWeight = round <= 1 ? 0.6 : round <= 2 ? 0.45 : 0.25;
...
function simulateBracket(
  ...
  const regionResults = {};
  REGIONS.forEach((r) => {
    regionResults[r] = simulateRegion(
      r,
      nodesByRegion[r],
      torvik,
      model,
  ...
  return {
    ...
    finalFour: {
      game1: {
        ...
        teamB: ff1B,
        winner: ff1Win,
        prob: ff1Prob >= 0.5 ? ff1Prob : 1 - ff1Prob,
      },
      game2: {
        teamA: ff2A,
        teamB: ff2B,
        winner: ff2Win,
        prob: ff2Prob >= 0.5 ? ff2Prob : 1 - ff2Prob,
      },
    },
    championship: {
      teamA: ff1Win,
...
export default async function handler(req, res) {
  ...
  try {
    const { graph, torvik, form, trans, injuryMap, oppBarthag, actualResults } =
      getData();
