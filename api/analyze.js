...
function getData() {
  ...
  _cache = { graph, torvik, form, injuryMap, transPairs, nodeByName, edgesByNode, oddsData };
  return _cache;
}
...
function routerNode(state) {
  const { graph, torvik, form, injuryMap, transPairs, nodeByName, edgesByNode, oddsData } = getData();
  ...
  return { team_a_data: teamAData, team_b_data: teamBData, matchup_data: matchupData, odds_data: oddsContextStr };
}
...
Output ONLY the analysis text — no notes, no meta-commentary, no self-referential statements about following instructions, no "Note:" lines.`;
  ...
  const user = `Synthesize these expert analyses into a final matchup prediction.
...
Write 3-4 sentences: the decisive factor, the key risk, and one thing that could flip the outcome. If market and model diverge by more than 8 points, call that out explicitly. Cite specific stats. Do not add any note, disclaimer, or commentary after the analysis.`;
